...
function tarih(ts) {
    ...
    if (typeof ts === "string" && ts.length <= 4) return "-";
    if (ts instanceof Date) ts = ts.getTime();
    if (ts.toString().length === 10) ts = ts * 1000;
    let d = new Date(ts);
    if (isNaN(d.getTime())) return "-";
    let gun = d.getDate().toString().padStart(2, '0');
    let ay = (d.getMonth() + 1).toString().padStart(2, '0');
    let yil = d.getFullYear();
    let saat = d.getHours().toString().padStart(2, '0');
    let dakika = d.getMinutes().toString().padStart(2, '0');
    return `${gun}.${ay}.${yil} ${saat}:${dakika}`;
}

// === Aranma alanlarını normalize et (backend farklı isimler döndürebilir) ===
function normalizeAranma(raw) {
    if (!raw) return null;
    let a = raw;
    if (typeof a === "string") {
        try { a = JSON.parse(a); } catch { a = {}; }
    }
    const durum = !!(a.durum ?? a.aktif ?? a.active);
    let baslangic = a.baslangic ?? a.suansaat ?? a.start ?? a.started_at ?? null;
    let bitis = a.bitis ?? a.saat ?? a.end ?? a.ends_at ?? null;
    const sebep = a.sebep ?? a.neden ?? a.reason ?? "";

    if (typeof baslangic === "string" && baslangic.trim() !== "") {
        const n = Number(baslangic);
        if (!Number.isNaN(n)) baslangic = n;
    }
    if (typeof bitis === "string" && bitis.trim() !== "") {
        const n = Number(bitis);
        if (!Number.isNaN(n)) bitis = n;
    }

    return { durum, sebep, baslangic, bitis };
}

// === EKLENDİ: Local cache'i güncelleyen helper ===
function updateLocalWanted(citizenid, wantedObj) {
    if (!citizenid) return;
    for (let i = 0; i < playerData.length; i++) {
        const row = playerData[i];
        if (row && row.citizenid === citizenid) {
            // NUI açılışında normalize ediyoruz; burada string saklamak daha uyumlu
            playerData[i].aranma = JSON.stringify(wantedObj || {});
            break;
        }
    }
}

function safeGetCharinfo(obj) {
    let c = obj.charinfo;
    if (!c) return {firstname: "", lastname: ""};
    if (typeof c === "string") { try { c = JSON.parse(c); } catch { c = {}; } }
    return c;
}
function safeGetAccounts(obj) {
    let a;
    if (obj.accounts !== undefined) {
        a = obj.accounts;
    } else if (obj.bank !== undefined) {
        a = { bank: obj.bank };
    } else {
        a = { bank: 0 };
    }
    if (typeof a === "string") {
        try { a = JSON.parse(a); } catch { a = { bank: 0 }; }
    }
    return a;
}
    ...
    $.post('http://'+resourceName+'/sorgula', JSON.stringify({data: data, tip: tip}), function(cbData) {
        ...
        if (playerData.length) {
            ...
            for (let index = 0; index < playerData.length; index++) {
                ...
                let charinfo = safeGetCharinfo(element);
                let name = (safe(charinfo.firstname) + " " + safe(charinfo.lastname)).trim();
                if (tip == "plaka") {
                    name = name + " (" + safe(element.plate) + ")";
                } else if (tip == "numara") {
                    name = name + " (" + safe(element.phone_number) + ")";
                }
                htmlSorgu = htmlSorgu + `
...
$(document).on("click",".kullanici",function() {
    ...
    let charinfo = safeGetCharinfo(userData);
    let g = (safe(userData.gender) || safe(userData.sex) || "").toString().toLowerCase();
    let userGender = ["m","male","e","erkek"].includes(g)
        ? lang["genderM"]
        : lang["genderF"];

    let aranma = normalizeAranma(userData.aranma);

    zanliIsim = (safe(charinfo.firstname) + " " + safe(charinfo.lastname)).trim();
    ...
    if (aranma && aranma.durum) {
        showUi(".aranma-kutu", "flex")
        $(".aranma-kutu").html(`
            <div class="aranma-neden"><i class="fas fa-exclamation-triangle fa-lg"></i>${safe(aranma.sebep)}</div>
            <div class="aranma-sag">
                <div class="aranma-baslangic"><i class="fas fa-clock fa-lg"></i>${tarih(aranma.baslangic)}</div>
                <div class="aranma-saat"><i class="fas fa-unlock-alt fa-lg"></i>${tarih(aranma.bitis)}</div>
            </div>
        `)
    ...
    $.post('http://'+resourceName+'/photo', JSON.stringify({data: secilenCitizenid}), function(cbData) {
        ...
        if (!userPhoto) userPhoto = "img/avatar.png";
        let userBank = safeGetAccounts(userData);
        hideUi("#arac-bilgi")
        ...
        $("#kullaniciBilgi").html(`
            <div class="sorgu-sag-alt-sol">
                ...
                <div class="bilgi"><div class="bilgi-sol">${lang["name"]}</div><div class="bilgi-sag">${safe(charinfo.firstname)}</div></div>
                <div class="bilgi"><div class="bilgi-sol">${lang["lastname"]}</div><div class="bilgi-sag">${safe(charinfo.lastname)}</div></div>
            </div>
            <div class="sorgu-sag-alt-sag">
                <div class="bilgi"><div class="bilgi-sol">${lang["phoneNumber"]}</div><div class="bilgi-sag">${safe(userData.phone_number)}</div></div>
                <div class="bilgi"><div class="bilgi-sol">${lang["bankMoney"]}</div><div class="bilgi-sag">${Intl.NumberFormat('en-US').format(userBank.bank || 0)}$</div></div>
                <div class="bilgi"><div class="bilgi-sol">${lang["dob"]}</div><div class="bilgi-sag">${safe(charinfo.birthdate)}</div></div>
            </div>
    ...
    $.post('http://'+resourceName+'/sorgula', JSON.stringify({data: secilenCitizenid, tip: "arac"}), function(cbData) {
        ...
        if (playerVehicleData.length > 0) {
            for (let index = 0; index < playerVehicleData.length; index++) {
                ...
                let model = element.vehicle || lang["unknown"]+"...";
                let plate = element.plate || lang["unknown"]+"...";
                $(".arac-bilgi-kutu").append(`
                    <div class="arac-bilgi" data-id="${index}">
                        <div class="arac-bilgi-plaka">${plate}</div> 
                        <div class="arac-bilgi-isim">${model}</div>
                    </div>
                `)
            }
            showUi("#arac-bilgi")
...
$(document).on("click",".aranan-kutu-kapat", function() {
    ...
    // === EKLENDİ: listedeki cache’i de temizle
    updateLocalWanted(citizenid, {durum:false, sebep:"", baslangic:"", bitis:""});
...
$(document).on("click",".aranmabutton", function() {
    ...
    if (neden == "" ) {
        ...
        const bitisMs = Math.round(nowMs) + (saat * 86400000);

        $(".aranma-kutu").html(`
            <div class="aranma-neden"><i class="fas fa-exclamation-triangle fa-lg"></i>${neden}</div>
            <div class="aranma-sag">
                <div class="aranma-baslangic"><i class="fas fa-clock fa-lg"></i>${tarih(nowMs)}</div>
                <div class="aranma-saat"><i class="fas fa-unlock-alt fa-lg"></i>${tarih(bitisMs)}</div>
            </div>
        `)
        ...
        // === EKLENDİ: local cache'e de yaz (saniye cinsinden)
        updateLocalWanted(secilenCitizenid, {
            durum: true,
            sebep: neden,
            baslangic: Math.floor(nowMs/1000),
            bitis: Math.floor(bitisMs/1000)
        });
    }
});
...
$(document).on("click","#aranmakaldir", function() {
    ...
    // === EKLENDİ: local cache'i temizle
    updateLocalWanted(secilenCitizenid, {durum:false, sebep:"", baslangic:"", bitis:""});
});
...
window.addEventListener('message', (event) => {
    if (event.data.type === 'ilk-bilgi') {
        ...
        if (event.data.data.accounts && event.data.data.accounts.bank !== undefined) {
            $("#kullanici-banka").html(
                '<i class="fas fa-wallet fa-sm"></i> ' + event.data.data.accounts.bank + ' $'
            );
        } else {
            $("#kullanici-banka").html(
                '<i class="fas fa-wallet fa-sm"></i> 0 $'
            );
        }
